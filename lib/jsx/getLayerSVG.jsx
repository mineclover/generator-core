...
if (typeof cssToClip === 'undefined') {
  var _runCopyCSSFromScript = true
...
svg.HTMLEncode = (str) => {
  ...
  for (i = 0; i < str.length; ++i) {
    var c = str[i]
    result[i] = (c < 'A' && c !== '\r') || c > '~' || (c > 'Z' && c < 'a') ? `&#${c.charCodeAt()};` : str[i]
  }
  return result.join('')
}
...
svg.setCurrentLayer = function (theLayer) {
  if (typeof theLayer === 'number') {
    ...
    typeof theLayer === 'object' && // Check for DOM layer
...
svg.getGradient = function (useLayerFX) {
  ...
  if (gradInfo && colorStops) {
    var i,
      globalOpacity = gradInfo.opacity
    ...
    for (var c in colorStops) {
      svgStops.push(
        '  <stop offset="' +
          Math.round(colorStops[c].location) +
          '%"' +
          ' stop-color="' +
          colorStops[c].colorString(true) +
          '" stop-opacity="' +
          (colorStops[c].m / 100) * globalOpacity +
          '" />',
      )
    }
...
svg.replaceKeywords = (filterStr, params) => {
  var i,
    replaceList = filterStr.match(/[$](\w+)[$]/g)
  if (replaceList) {
...
svg.addColorOverlay = function () {
  ...
  if (overDesc?.getVal('enabled') && this.getLayerAttr('layerFXVisible')) {
    var params = {
      filterTag: `Filter_${this.filterID++}`,
...
svg.addInnerShadow = function () {
  ...
  if (inshDesc?.getVal('enabled') && this.getLayerAttr('layerFXVisible')) {
    ...
    var params = {
      filterTag: `Filter_${this.filterID++}`,
      dx: stripUnits(offset[0]),
      dy: stripUnits(offset[1]),
...
svg.addDropShadow = function () {
  ...
  if (dsInfo) {
    ...
    if (agmDesc?.getVal('strokeEnabled') && (strokeWidth = agmDesc.getVal('strokeStyleLineWidth'))) {
    ...
    var params = {
      ...
      dx: stripUnits(dsInfo.xoff),
      dy: stripUnits(dsInfo.yoff),
...
svg.getShapeLayerSVG = function () {
  var self = this
  var agmDesc = this.currentLayer.getLayerAttr('AGMStrokeStyleInfo')
  var capDict = { strokeStyleRoundCap: 'round', strokeStyleButtCap: 'butt', strokeStyleSquareCap: 'square' }
  var joinDict = { strokeStyleBevelJoin: 'bevel', strokeStyleRoundJoin: 'round', strokeStyleMiterJoin: 'miter' }

  function hasStroke() {
    return agmDesc?.getVal('strokeEnabled')
  }
  ...
  function addStroke() {
    ...
    if (fxDesc?.getVal('enabled') && fxDesc.getVal('paintType') === 'solidColor') {
  ...
  if (!agmDesc || agmDesc?.getVal('fillEnabled')) {
    if (gradientID) {
...
svg.getTextLayerSVG1 = function (fillColor) {
  ...
  var xfm = () => {}
  var midval = () => {} // For shutting up JSHint
  ...
  if (textDesc) {
    ...
    if (textXform) {
      ...
      if (!(xx === 1 && xy === 0 && yx === 0 && yy === 1 && tx === 0 && ty === 0)) {
        midval = (key0, key1, desc, op) => op(stripUnits(desc.getVal(key0)), stripUnits(desc.getVal(key1))) / 2.0
        // Find the vector representing the bottom left corner of
        // the original (untransformed) text bounds centered on the origin
        var obx = -midval('left', 'right', originalTextBounds, (a, b) => b - a)
        var oby = midval('top', 'bottom', originalTextBounds, (a, b) => -b - a)
        ...
        var cbx = midval('left', 'right', boundsDesc, (a, b) => a + b)
        var cby = midval('top', 'bottom', boundsDesc, (a, b) => a + b)
    ...
    if (!transformMatrixUsed) {
      ...
      if (textShapeDesc.getVal('char') === 'box') {
        ...
        if (lineBreaks) {
          textBottom -=
            stripUnits(this.getLayerAttr('textKey.bounds.bottom')) - stripUnits(originalTextBounds.getVal('bottom'))
        } else {
...
svg.getImageLayerFileRefSVG = function () {
  ...
  var suffix = name.lastIndexOf('.') >= 0 ? name.slice(name.lastIndexOf('.')).toLowerCase() : null
...
svg.walkLayerGroup = function (processAllLayers) {
  function isSVGLayerKind(kind) {
    return cssToClip.isCSSLayerKind(kind) || kind === kAdjustmentSheet || kind === kSmartObjectSheet
  }
...
svg.getGroupLayerSVG = function (processAllLayers) {
  var i,
    groupLayers = this.walkLayerGroup(processAllLayers)
...
svg.processLayer = function (layer) {
  ...
  switch (this.currentLayer.layerKind) {
    case kVectorSheet:
      this.getShapeLayerSVG()
      return true
    case kTextSheet:
      this.getTextLayerSVG()
      return true
    case kSmartObjectSheet:
    case kPixelSheet:
      this.getImageLayerSVG()
      return true
    case kAdjustmentSheet:
      this.getAdjustmentLayerSVG()
      return true
...
svg.findActualBounds = function () {
  ...
  for (i = 0; i < layers.length; ++i) {
    if (typeof layers[i] !== 'number' && layers[i].layerKind in contentLayerKinds) {
      ...
      if (layers[i].layerKind === kVectorSheet) {
        ...
        if (agmDesc?.getVal('strokeEnabled')) {
...
svg.createSVGText = function () {
  ...
  if (fixBoundsAvailable) {
    ...
    var boundsParams = {
      width: ((bounds[2] - bounds[0] + halfStrokeWidth) * params.layerScale).asCSS(),
